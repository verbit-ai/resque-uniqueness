...
RSpec.describe Resque::Uniqueness::ResqueExtension do
  describe '.remove_queue' do
    ...
    before do
      ...
      allow(Resque::Uniqueness::Job).to receive(:remove_queue)
    end

    its_block { is_expected.to send_message(data_store_instance, :remove_queue) }
    its_block { is_expected.to send_message(Resque::Uniqueness::Job, :remove_queue) }
    ...
    context 'when resque inline' do
      ...
      its_block { is_expected.to send_message(data_store_instance, :remove_queue) }
      its_block { is_expected.not_to send_message(Resque::Uniqueness::Job, :remove_queue) }
      it { is_expected.to eq :response }
    end
  end
  ...
  describe '.remove_delayed_job' do
    ...
    before { allow(Resque::Uniqueness::Job).to receive(:unlock_schedule) }

    its_block { is_expected.to send_message(Resque::Uniqueness::Job, :unlock_schedule) }
    ...
    context 'when resque inline' do
      ...
      its_block { is_expected.not_to send_message(Resque::Uniqueness::Job, :unlock_schedule) }
      it { is_expected.to eq 0 }
    end
  end
  ...
  describe '.remove_delayed_job_from_timestamp' do
    ...
    before do
      ...
      allow(Resque::Uniqueness::Job).to receive(:unlock_schedule)
    end
